...
export class Degree {
    ...
    async getDegreeImage(server: Server): Promise<Image> {
        var degreeImage = await downloadFile(`https://bestdori.com/assets/${server.serverName}/thumb/degree_rip/${this.baseImageName[server.serverId]}.png`)
        return loadImage(degreeImage)
    }
    async getDegreeFrame(server: Server): Promise<Image | Canvas> {
        var frameName = this.degreeType[server.serverId] + "_" + this.rank[server.serverId]
        if (frameName == "none_none") {//这个为空底图
            return (createCanvas(1, 1))
        }
        var degreeFrame = await downloadFileCache("https://bestdori.com/assets/" + server.serverName + "/thumb/degree_rip/" + frameName + ".png")
        return loadImage(degreeFrame)
    }
    async getDegreeIcon(server: Server): Promise<Image | Canvas> {
        var iconName = this.iconImageName[server.serverId] + "_" + this.rank[server.serverId]
        if (iconName == "none_none") {//这个为空底图
            return (createCanvas(1, 1))
        }
        var degreeIcon = await downloadFileCache("https://bestdori.com/assets/" + server.serverName + "/thumb/degree_rip/" + iconName + ".png")
