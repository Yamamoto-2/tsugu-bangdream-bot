...
export class Item {
    ...
    async getItemImage(server?: Server): Promise<Image> {
        ...
        if (this.typeName == 'material') {
            var itemImage = await downloadFileCache(`https://bestdori.com/assets/${server.serverName}/thumb/material_rip/${this.typeName}${formatNumber(this.resourceId, 3)}.png`)
        }
        else if (this.typeName == 'star') {
            var itemImage = await downloadFileCache(`https://bestdori.com/assets/${server.serverName}/thumb/common_rip/star.png`)
        }
        else {
            var itemImage = await downloadFileCache(`https://bestdori.com/assets/${server.serverName}/thumb/common_rip/${this.typeName}${this.resourceId}.png`)
        }
